...
{
	/// <summary>
	/// Filterable tree list component. Define content through <see cref="TreeNode_FilterBox"/>.
	/// </summary>
	internal class FilterTreeBox
	{
		private float _columnWidthRatio;

		/// <summary>
		/// Gets or sets the width ratio of the value column.
		/// </summary>
		/// <value>
		/// Ratio supplied as a value between 0 and 1: 0.5 for 50%.
		/// </value>
		public float ValueColumnRatio
		{
			get { return _columnWidthRatio; }
			set { _columnWidthRatio = value; }
		}
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="FilterTreeBox"/> component, used to render a tree interface with built-in filter bar.
		/// </summary>
		/// <param name="roots">Node-structure to display, this contains the entire interface definition.</param>
		...
		public void Draw(Rect inRect)
		{
			...
			if (Widgets.ButtonText(new Rect(inRect.xMax - clearButtonSize - 5, curY, clearButtonSize, clearButtonSize), "X"))
				searchText = "";
			...
			for (; currentIndex < count; currentIndex++)
			{
				...
				if (lineListing.Node(node, node.nestDepth, listbox.width * _columnWidthRatio, false))
				{
					UpdateVisibleNodes();
					break;
				}
